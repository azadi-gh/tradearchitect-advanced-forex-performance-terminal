...
export function AppSidebar(): JSX.Element {
  ...
  const menuItems = [
    ...
    { title: "Risk Lab", icon: Calculator, path: "/risk", tooltip: "Position & Return Calculators" },
      ...
      <SidebarFooter className="p-4 border-t border-border bg-muted/20">
        <Button variant="default" className="w-full justify-start gap-2 shadow-primary hover:scale-[1.02] transition-transform" asChild>
          <Link to="/journal">
            ...
            <span className="font-bold uppercase text-[10px] tracking-widest">New Execution</span>
          </Link>
        </Button>
        <div className="mt-4 flex flex-col gap-1 px-2">
          <div className="flex items-center gap-2">
            <Settings className="h-3 w-3 text-muted-foreground" />
            <span className="text-[10px] text-muted-foreground font-black uppercase tracking-tighter">System Status</span>
          </div>
          <p className="text-[9px] text-emerald-500 font-bold uppercase ml-5">v1.2: Intelligence & Stability</p>
        </div>
      </SidebarFooter>
    </Sidebar>
  );
