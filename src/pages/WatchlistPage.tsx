...
export function WatchlistPage() {
  ...
  const { data: watchlist } = useQuery<any>({ queryKey: ['watchlist'], queryFn: () => api('/api/watchlist') });
  const { data: stats } = useQuery<any>({ queryKey: ['dashboard-stats'], queryFn: () => api('/api/dashboard/stats') });
  ...
  const filteredMarket = MARKET_DIRECTORY.filter(m => m.symbol.toLowerCase().includes(search.toLowerCase()));
  return (
    <AppLayout container>
        ...
        <div className="py-8 md:py-10 lg:py-12 space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="relative w-full md:w-96">
              ...
              <Input placeholder="Search symbols..." className="pl-10 h-12 bg-secondary/50 font-bold" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    {watchlist?.pairs?.map((p: string) => (
                      <div key={p} className="flex items-center justify-between p-4 border-b last:border-0 hover:bg-primary/5 transition-colors group">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 opacity-0 group-hover:opacity-100" onClick={() => toggleWatchlist.mutate(p)}><Check className="h-4 w-4" /></Button>
                        </div>
                      </div>
                    ))}
                  </ScrollArea>
            ...
            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredMarket.map((item) => {
                ...
                return (
                  <motion.div key={item.symbol} layout>
                    <Card className="group border-2 hover:border-primary/40 transition-all duration-300 overflow-hidden">
                      <CardContent className="p-6 space-y-4">
                        <div className="flex justify-between items-start">
                          ...
                          <Button variant={watchlist?.pairs?.includes(item.symbol) ? "secondary" : "outline"} size="icon" onClick={() => toggleWatchlist.mutate(item.symbol)}>
                            {watchlist?.pairs?.includes(item.symbol) ? <Star className="h-4 w-4 fill-primary text-primary" /> : <Plus className="h-4 w-4" />}
                          </Button>
                        </div>
                        <div className="flex gap-2">
                        ...
                        <div className="pt-4 border-t flex justify-between items-center">
                          <Button variant="ghost" className="text-[10px] font-black uppercase tracking-widest h-8 px-2" asChild>
                            <a href={`/journal?symbol=${item.symbol}`}>Log Execution</a>
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
